...
void main()
{
	vec3 albedo = vec3(1.f, 1.f, 1.f);//pow(texture(albedo_map, Tex_Coords).rgb, vec3(2.2)); //gamma correction
    float metalness  = 1.f;//texture(metalness_map, Tex_Coords).r;
    //float roughness = texture(roughness_map, Tex_Coords).r;
    float ao = 1.f;//texture(ao_map, Tex_Coords).r;
